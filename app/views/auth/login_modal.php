...
<dialog id="login_modal" class="modal">
    <div class="modal-box p-0 max-w-md bg-white">
        ...
        <button type="button"
            class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 text-gray-500 hover:text-gray-700 z-10"
            onclick="login_modal.close()">✕</button>
        <!-- Mensaje de error -->
            ...
            <div class="form">
                ...
                <div class="inputForm">
                    ...
                    <div class="relative w-full">
                        <input type="password" id="login_password" class="input w-full pr-10" name="clave"
                            placeholder="Ingresa tu contraseña" required />
                        <i id="toggle_login_password"
                            class="fas fa-eye absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 cursor-pointer hover:text-gray-700 z-10"></i>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            e.preventDefault(); // ← EVITA RECARGAR Y CERRAR EL MODAL
            ...
            // Enviar AJAX
            fetch('<?php echo BASE_URL; ?>/auth/loginAjax', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data.ok) {
                    ...
                    errorText.textContent = data.mensaje;
